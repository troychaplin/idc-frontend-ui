...
export const CardContent = ({ text, link, isExcerpt = false, hideMobile = false }: CardContentProps) => {
    ...
    <p className={`${hideMobileClass} mt-2 text-base font-normal leading-7 md:mt-3`}>
      {isExcerpt && text && text.length > 170 ? (
        <>
          ...
          {link && (
            <a href={link} className="ml-1 font-semibold text-idc-orange-700 hover:underline">
              More
            </a>
          )}
        </>
