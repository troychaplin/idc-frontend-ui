...
export const Section = ({ children, maxWidth = '5xl', bgType = 'white' }: SectionProps) => {
  return (
    <section
      className={`ui-section ui-section--${bgType} ${uiBgTypes[bgType]} w-screen ml-offset-center`}
    >
      <div className={`ui-max-w-child-${maxWidth} px-0 md:px-8 ui-prose-first-last`}>
        {children}
      </div>
    </section>
  );
